...
def evaluate(respth='/home/yexiaoqi/PycharmPro/zyd/rawscan_parsing/',
                 dspth='/home/yexiaoqi/PycharmPro/zyd/rawscan_normalize/', cp='Seg_79999_iter.pth'):

    if not os.path.exists(respth):
        os.makedirs(respth)

    image_list = sorted(glob.glob(dspth + "*.jpg"))
    ...
    with torch.no_grad():
        for i in range(0, len(image_list)):   # len(image_list)
            ...
            vis_im = vis_parsing_maps(h, w, image, parsing, stride=1, save_path=osp.join(respth, name), save_im=True)
            print(i, image_path)
            ...
            Image.fromarray(crop).save("/home/yexiaoqi/PycharmPro/zyd/rawscan_parsing/"+name)

            # Image.fromarray(crop).save("/home/zhang/zydDataset/faceRendererData/crop_p/"+name)
